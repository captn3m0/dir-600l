...
Page.prototype =
{
	...
	IsDirty: function()
	{
		...
		for (var i=0; i<this.cfg.length; i+=1)
		{
			if (this.org[i].uid !== this.cfg[i].uid ||
				...
				this.org[i].format !== this.cfg[i].format ||
	...
	TimeString: function(i)
	{
		...
		if (this.cfg[i].format=="24")
                {
                        Start.To24Hrs();
                        End.To24Hrs();
                }
                else if (this.cfg[i].format=="12")
                {
                        Start.To12Hrs();
                        End.To12Hrs();
                }
	...
	OnClick24Hours: function()
	{
		...
		OBJ("schtimeformat").disabled=OBJ("schstarthrs").disabled=OBJ("schstartmin").disabled=OBJ("schstartapm").disabled =
	...
	OnChangeFormat: function()
        {
                var tmp_format = OBJ("schtimeformat").value;

                if (tmp_format=="24")
                {
                        OBJ("schstartapm").disabled = OBJ("schendapm").disabled = true;
                }
                else if (tmp_format=="12")
                {
	...
	OnClickSchSubmit: function()
	{
		var e = {
			...
			format: OBJ("schtimeformat").value,
			start:	{
		...
		if (OBJ("schtimeformat").value=="12")
                {
			if (isNaN(e.start.hour) || isNaN(e.start.min) || isNaN(e.end.hour) || isNaN(e.end.min) ||
				e.start.hour < 1 || e.start.hour > 12 || e.start.min < 0 || e.start.min > 59 ||
				e.end.hour < 1 || e.end.hour > 12 || e.end.min < 0 || e.end.min > 59)	
			{
				alert("<?echo i18n("The schedule is not valid.");?>");
				return;
			}
		}
                else if (OBJ("schtimeformat").value=="24")
                {
			if (isNaN(e.start.hour) || isNaN(e.start.min) || isNaN(e.end.hour) || isNaN(e.end.min) ||
                                e.start.hour < 0 || e.start.hour > 23 || e.start.min < 0 || e.start.min > 59 ||
                                e.end.hour < 0 || e.end.hour > 23 || e.end.min < 0 || e.end.min > 59)
                        {
                                alert("<?echo i18n("The schedule is not valid.");?>");
                                return;
                        }
	...
	OnEdit: function(i)
	{
		...
		this.OnClickSelectDays();
		OBJ("sch24hrs").checked = false;
		COMM_SetSelectValue(OBJ("schtimeformat"), this.cfg[i].format);
		this.OnClick24Hours();
		...
		if (OBJ("schtimeformat").value=="24")
                ...
                else if (OBJ("schtimeformat").value=="12")
                {
                        Start.To12Hrs();
                        End.To12Hrs();
                }
