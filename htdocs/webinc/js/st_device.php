...
<?
		$wlan2 = 0;
		foreach("/phyinf")      {       if ( query("uid") == "WLAN-2" ) $wlan2 = 1;     }
		if ($wlan2 == 0)        echo "\t\tif (!this.InitWLAN(\"WLAN-1\")) return false;\n";
		else
		{
			echo "\t\tif (!this.InitWLAN(\"WLAN-1\")) return false;\n";
			echo "\t\tif (!this.InitWLAN(\"WLAN-2\")) return false;\n";
		}
?> 	
<?
		if ($layout=="router")
...
?>		
	...
	InitWLAN: function ( wlan )
	{
		var str_2 = "";
		var wifi_phyinf_path = PXML.FindModule("WIFI.PHYINF");
		var phy_wlan_path = GPBT(wifi_phyinf_path, "phyinf", "uid", wlan, false);
		...
		var rwlan = GPBT(rphy+"/runtime", "phyinf", "uid", wlan, false);
		
		if ((!wifi_path)||(!phy_wlan_path))
		{
			BODY.ShowAlert("InitWLAN(" + wlan +") ERROR!!!");
			return false;
		}
		...
		OBJ("st_wireless_radio"+str_2).innerHTML  = XG(phy_wlan_path+"/active")== "1" ? "<?echo i18n("Enabled");?>":"<?echo i18n("Disabled");?>";
		...
		{
		   case "bgn":
		   		OBJ("st_80211mode"+str_2).innerHTML  = "<?echo i18n("Mixed 802.11n, 802.11g and 802.11b");?>";
                check_bandwidth = 1;
				break;
		   case "bg":
		   		OBJ("st_80211mode"+str_2).innerHTML  = "<?echo i18n("Mixed 802.11g and 802.11b");?>";
				break;
		   case "gn":
		   		OBJ("st_80211mode"+str_2).innerHTML  = "<?echo i18n("Mixed 802.11n and 802.11g");?>";
                check_bandwidth = 1;
				break;
		   case "an":
		   		OBJ("st_80211mode"+str_2).innerHTML  = "<?echo i18n("Mixed 802.11n and 802.11a");?>";
                check_bandwidth = 1;
				break;
		   case "n":
		   		OBJ("st_80211mode"+str_2).innerHTML  = "<?echo i18n("802.11n only");?>";
                check_bandwidth = 1;
				break;
		   case "b":
		   		OBJ("st_80211mode"+str_2).innerHTML  = "<?echo i18n("802.11b only");?>";
				break;
		   case "g":
		   		OBJ("st_80211mode"+str_2).innerHTML  = "<?echo i18n("802.11g only");?>";
				break;
		   case "a":
		   		OBJ("st_80211mode"+str_2).innerHTML  = "<?echo i18n("802.11a only");?>";
				break;
		}
		...
		OBJ("st_Channel_Width"+str_2).innerHTML  = string_bandwidth;
		OBJ("st_Channel"+str_2).innerHTML  = XG  (rwlan+"/media/channel");
		
		var channel = XG  (rwlan+"/media/channel");
		OBJ("st_Channel"+str_2).innerHTML  = channel ? channel : "N/A";
		
		OBJ("st_SSID"+str_2).innerHTML  = COMM_EscapeHTMLSC(XG(wifi_path+"/ssid"));
		...
		OBJ("st_WPS_status"+str_2).innerHTML  = string_WPS;
		...
		OBJ("st_security"+str_2).innerHTML  = string_security;
	    return true;
	},
	DHCP_Renew: function()
	{
